...
tabs = st.tabs([
    ...
    "계약서 AI 판독", "🎙️ 녹취록 AI 변환", "증거 및 진정서", "📍 노동 지원 기관"
])
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("따뜻한 답변을 고민하고 있습니다..."):
                try:
                    ...
                    ai_response = f"⏳ 대기자가 많아 AI가 잠시 숨을 고르고 있습니다. 10초 뒤에 다시 말씀해주세요!"
    ...
    if situation != "선택하세요":
        ...
        if situation == "1. 임금 체불 및 퇴직금 미지급":
            ...
            if contract_type == "3.3% 프리랜서 위임 계약":
                ...
                st.info(f"📊 **[계약 형태 분석 적용]** 현재 선택하신 **'{contract_type}'**은 프리랜서 계약이 아니므로 이 진단 유형에 적합하지 않습니다.")
            ...
            if dismissal_status == "30일 미만 남겨두고 기습 해고 통보 받음 (즉시 해고)":
                if is_over_3months:
                    st.error(f"🚨 **해고예고수당 (30일분 통상임금) 청구 대상 확정!**\n\n귀하는 근무 기간이 {working_months}개월(3개월 이상)이면서 30일 전에 명확한 예고 없이 당장 해고를 당하셨습니다. 고용주는 **반드시 30일분 이상의 통상임금**을 지급해야 할 법적 의무가 생깁니다.")
                else:
                    st.warning(f"⚠️ **해고예고수당 제외 대상 발견:** 총 근무 기간이 3개월 미만({working_months}개월)인 경우 법적인 해고예고수당 지급 예외 사유에 해당합니다.")
                
                if emp_count < 5:
                    st.warning(f"📌 **부당해고 구제신청 불가 (5인 미만):** 현재 사업장 규모가 {emp_count}명이므로 노동위원회에 '부당해고 구제신청'은 불가합니다.")
                else:
                    st.success(f"🎉 **부당해고 구제신청 가능 (5인 이상):** 상시 근로자가 {emp_count}명으로 조건을 만족합니다. 노동위원회에 **부당해고 구제신청**을 넣어 복직 및 임금을 청구할 수 있습니다.")
            elif dismissal_status == "30일 전에 미리 해고 예고 통보 받음":
                st.info("정상적인 해고 예고 절차(30일 전 통보)가 이행된 상태이므로 해고예고수당은 발생하지 않습니다.")

        elif situation == "5. 산업재해 (업무 중 다침)":
            st.error("🚨 진단: 업무상 사고/질병은 근로자의 과실 유무나 규모와 상관없이 보상받아야 합니다. 산재를 직접 신청하세요.")

        elif situation == "6. 외국인 노동자 차별 및 협박":
            st.error("🚨 진단: 미등록 상태를 약점 잡아 협박하는 것은 명백한 인권 침해 및 범죄입니다. 노동청 신고 시 출입국 통보 의무가 면제됩니다.")
    ...
    if uploaded_img:
        ...
        with st.spinner("AI가 계약서 내용을 꼼꼼히 읽고 분석하고 있습니다... (약 5~10초 소요)"):
            try:
                ...
                2. 근무 장소, 출퇴근 시간이 명확히 고정되어 있는지
                3. 최저임금 위반, 부당한 손해배상 청구 등 독소 조항 여부

                [출력 형식]
                결과는 반드시 "[안전]", "[주의]", "[위험]" 중 하나의 상태로 시작하고, 
                그 이유와 발견된 조항의 문제점을 친절하게 요약해서 알려주세요.
                """
                response = model.generate_content([image, analyze_prompt])
                st.success("✅ AI 판독이 완료되었습니다!")
                st.info(response.text)
            except Exception as e:
                st.error(f"🚨 분석 중 오류가 발생했습니다. (내용: {e})")

# -------------------------------------------------------------
# TAB 4: 녹취록 AI 변환 (🔥 진짜 오디오 인식 기능 탑재)
# -------------------------------------------------------------
with tabs[4]:
    st.markdown("### 🎙️ 폭언/협박 녹취록 AI 변환 (STT)")
    st.write("사장님의 폭언, 해고 통보 등 녹음된 음성 파일(.mp3)을 올리면 AI가 직접 듣고 텍스트로 받아 적어줍니다.")
    
    # 세션 상태에 변환된 텍스트를 임시로 저장하기 위한 공간 마련
    ...
    if uploaded_audio:
        # 업로드한 음성을 화면에서 직접 재생해 볼 수 있도록 오디오 플레이어 렌더링
        st.audio(uploaded_audio)
        
        # 버튼을 누르면 AI 분석 시작
        if st.button("음성 파일 분석 시작", type="primary"):
            with st.spinner("AI가 음성을 듣고 텍스트로 변환하고 있습니다. (파일 길이에 따라 10초 이상 소요될 수 있습니다)"):
                try:
                    # 1. 파일에서 바이트(bytes) 데이터 읽어오기
                    audio_bytes = uploaded_audio.read()
                    
                    # 2. 제미나이(Gemini)에게 전달할 오디오 데이터 규격 맞추기
                    audio_part = {
                        "mime_type": uploaded_audio.type, # "audio/mp3" 등
                        "data": audio_bytes
                    }
                    
                    # 3. AI에게 내릴 STT(음성 인식) 프롬프트 작성
                    stt_prompt = """
                    이 음성 파일의 내용을 들리는 그대로 정확하게 텍스트로 변환해 주세요. 
                    노동 분쟁 증거자료로 쓸 예정이므로, 욕설, 폭언, 협박, 혹은 거친 표현이 있더라도 절대 순화하거나 요약하지 말고 '있는 그대로 정확하게' 전사(Transcription)해 주세요.
                    """
                    
                    # 4. 제미나이 모델 호출하여 소리 분석 요청
                    response = model.generate_content([stt_prompt, audio_part])
                    
                    # 분석 결과를 세션 상태에 저장하여 버튼이 눌린 후에도 텍스트가 날아가지 않도록 함
                    st.session_state.current_transcript = response.text
                    
                except Exception as e:
                    st.error(f"🚨 음성 분석 중 오류가 발생했습니다. (내용: {e})")

    # AI가 텍스트 변환을 완료하여 결과값이 존재할 때 화면에 출력
    if st.session_state.current_transcript:
        st.success("✅ AI 음성 인식 완료!")
        st.info(f"**[추출된 텍스트 내용]**\n\n{st.session_state.current_transcript}")
        ...
        if st.button("이 녹취록을 [증거 보관함]에 즉시 저장"):
            ...
            st.session_state.evidence_db.append({
                ...
                "유형": "음성 녹취 (STT 변환 완료)", 
                "정황": st.session_state.current_transcript
            })
            
            # 저장이 끝났으니 임시 변환값 초기화
            st.session_state.current_transcript = None
            st.success("증거 보관함에 성공적으로 저장되었습니다! '증거 및 진정서' 탭을 확인하세요.")
...
with tabs[6]:
    ...
    st.dataframe(
        ...
        column_config={
            "웹사이트": st.column_config.LinkColumn("공식 홈페이지", display_text="웹사이트 방문하기 🔗")
        },
        hide_index=True,
