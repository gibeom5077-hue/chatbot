...
with tabs[2]:
    ...
    emp_count = c1.number_input("상시 근로자 수", min_value=1, value=5)
    contract_type = c2.radio("계약 형태", ["일반 근로계약서", "3.3% 프리랜서 위임 계약", "구두 계약(계약서 없음)"])
    work_hours = c3.number_input("1주 소정 근로시간", min_value=1, value=16)
    ...
    if situation != "선택하세요":
        ...
        if contract_type == "구두 계약(계약서 없음)":
            st.warning("⚠️ **[경고] 계약서 미작성 상태 확인:** 현재 입력하신 '구두 계약' 역시 고용주에게 500만 원 이하의 벌금이 부과되는 명백한 불법입니다. 신고 시 이 사실을 함께 주장하세요.")

        # 2️⃣ 상황별 맞춤 진단 로직
        if situation == "1. 임금 체불 (월급, 퇴직금, 주휴수당 미지급)":
            st.error("🚨 진단: 사용자가 근로의 대가를 지급하지 않는 불법 행위입니다.")
            
            # 🔥 근로시간 조건 실시간 반영
            if work_hours < 15:
                st.info(f"📊 **[근로시간 분석 적용]** 귀하의 근로시간은 **주 {work_hours}시간**으로 '초단시간 근로자'에 해당합니다. ➡ **주휴수당 및 퇴직금은 청구할 수 없으나, 일한 만큼의 시급 체불은 100% 청구 가능합니다.**")
            else:
                st.success(f"📊 **[근로시간 분석 적용]** 귀하의 근로시간은 **주 {work_hours}시간**으로 주 15시간 이상입니다. ➡ **체불된 월급은 물론, 주휴수당과 퇴직금까지 모두 청구 대상입니다.**")
            
            st.markdown("""
            **💡 구체적인 해결 방법:**
            1. **증거 수집:** 급여통장 내역, 출퇴근 기록을 확보하세요.
            2. **내용증명 발송:** 체불 임금 독촉 내용증명을 우편 발송해 압박하세요.
            3. **노동청 진정 및 대지급금 신청:** 관할 고용노동청에 신고하고, 못 받은 돈은 국가가 대신 주는 대지급금 제도를 활용하세요.
            """)
            ...
            # 🔥 상시 근로자 수 실시간 반영
            if emp_count < 5:
                st.warning(f"📊 **[근로자 수 분석 적용]** 상시 근로자가 **{emp_count}명(5인 미만)**이므로, 원직 복직을 요구하는 '부당해고 구제신청'은 불가능합니다.")
                st.markdown("""
                **💡 5인 미만 유일한 해결 방법 (해고예고수당):**
                다만, 사장이 **최소 30일 전에 미리 해고를 예고하지 않고 당장 내일부터 나오지 말라고 했다면**, 5인 미만 사업장이라도 **30일분의 통상임금(해고예고수당)**을 청구할 수 있습니다.
                """)
            else:
                st.success(f"📊 **[근로자 수 분석 적용]** 상시 근로자가 **{emp_count}명(5인 이상)**이므로 부당해고 구제신청이 가능합니다.")
                st.error("🚨 진단: 5인 이상 사업장에서 서면 통지 없는 해고(카톡 통보 등)는 무조건 무효입니다.")
                st.markdown("""
                **💡 구체적인 해결 방법:**
                1. **절대 사직서 서명 금지:** "저는 계속 일하고 싶습니다"라는 문자를 명확히 남기세요.
                2. **노동위원회 구제 신청:** 해고일로부터 '3개월 이내'에 관할 노동위원회에 구제신청을 접수하여 해고 기간 동안의 월급을 받아내세요.
                """)
